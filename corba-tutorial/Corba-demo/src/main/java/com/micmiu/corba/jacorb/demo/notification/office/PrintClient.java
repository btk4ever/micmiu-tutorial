...
import demo.notification.office.Printer;
import demo.notification.office.PrinterHelper;
...
//				"org.jacorb.orb.giop.BiDirConnectionInitializer");
		...
		try {
			...
			channel = EventChannelHelper.narrow(nc.resolve(nc.to_name("notification.office")));

			printer = PrinterHelper.narrow(nc.resolve(nc.to_name("Office.Printer")));

			poa.the_POAManager().activate();

			// create and implicitly activate the client
			structuredPullConsumer = (StructuredPullConsumer) new PrintClient()._this(orb);

			// get the admin interface and the supplier proxy
			consumerAdmin = channel.default_consumer_admin();

			proxyPullSupplier = StructuredProxyPullSupplierHelper.narrow(
			...
			if (filterFactory == null) {
				...
				EventType[] eventTypes = new EventType[]{
						new EventType("Office", "Printed"), new EventType("Office", "Canceled")};
