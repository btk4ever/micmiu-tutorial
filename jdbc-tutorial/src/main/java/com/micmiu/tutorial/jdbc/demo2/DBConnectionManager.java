package com.micmiu.tutorial.jdbc.demo2;
...
public class DBConnectionManager {
		...
		while (propNames.hasMoreElements()) {
			...
			if (name.endsWith(".url")) {
				...
				try {
					...
					log(e, "错误的最大连接数限制: " + maxconn + " .连接池: " + poolName);
					max = 0;
				}
				try {
					min = Integer.valueOf(minconn).intValue();
				} catch (NumberFormatException e) {
					log(e, "错误的最小连接数限制: " + minconn + " .连接池: " + poolName);
					min = 0;
				}
	...
	private void loadDrivers(Properties props) {
		...
		while (st.hasMoreElements()) {
			...
			try {
				...
				log(e, "无法注册JDBC驱动程序: " + driverClassName);
			}
		}
	}
	...
	class DBConnectionPool {
		...
		private void monitor() {
			while (true) {
				// /每隔logInterval对连接池清理一次,删除无效连接或者长时没有使用的连接
